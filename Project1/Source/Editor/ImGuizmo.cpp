...
namespace ImGuizmo
{
  ...
  /* void LookAt(const float* eye, const float* at, const float* up, glm::mat4* m16)
   {
   ...
   public:
	  ...
	  vec_t() {}
	  vec_t(glm::vec4 a) { x = a.x, y = a.y, z = a.z, w = a.w; }
	  ~vec_t () {}
   ...
   public:
      ...
      matrix_t() {}
	  matrix_t(const glm::mat4 a)
	  {
		  memcpy(m16, glm::value_ptr(a), sizeof(a));
		  for (int i = 0; i < 4; ++i)
		  {
			  for (int j = 0; j < 4; ++j)
			  {
	  ...
	  {
		  glm::mat4 a;
		  memcpy(glm::value_ptr(a), m16, sizeof(m16));
		  return a;
	  }
   ...
   static void ComputeContext(const glm::mat4 *view, const glm::mat4 *projection, glm::mat4 *matrix, MODE mode)
   {
      gContext.mMode = mode;
   ...
   static void DrawTranslationGizmo(int type)
   {
      ...
      if (gContext.mbUsing)
      {
         ImVec2 sourcePosOnScreen = worldToPos(gContext.mMatrixOrigin, gContext.mViewProjection);
         ImVec2 destinationPosOnScreen = worldToPos(gContext.mModel.v.position, gContext.mViewProjection);
         vec_t dif(glm::vec4(destinationPosOnScreen.x - sourcePosOnScreen.x, destinationPosOnScreen.y - sourcePosOnScreen.y, 0.f, 0.f));
         dif.Normalize();
         dif *= 5.f;
         drawList->AddCircle(sourcePosOnScreen, 6.f, translationLineColor);
         drawList->AddCircle(destinationPosOnScreen, 6.f, translationLineColor);
   ...
   static void HandleAndDrawLocalBounds(float *bounds, matrix_t *matrix, float *snapValues, OPERATION operation)
   {
       ...
       vec_t bestAxisWorldDirection(glm::vec4(0));
       int axes[3];
   ...
   static int GetRotateType()
   {
      ...
      vec_t deltaScreen(glm::vec4(io.MousePos.x - gContext.mScreenSquareCenter.x, io.MousePos.y - gContext.mScreenSquareCenter.y, 0.f, 0.f));
   ...
   static void HandleTranslation(glm::mat4 *matrix, float *deltaMatrix, int& type, float *snap)
   {
      ImGuiIO& io = ImGui::GetIO();
   ...
   static void HandleScale(glm::mat4 *matrix, float *deltaMatrix, int& type, float *snap)
   {
      ImGuiIO& io = ImGui::GetIO();
   ...
   static void HandleRotation(glm::mat4 *matrix, float *deltaMatrix, int& type, float *snap)
   {
      ImGuiIO& io = ImGui::GetIO();
   ...
   void DecomposeMatrixToComponents(const glm::mat4 *matrix, float *translation, float *rotation, float *scale)
   {
      matrix_t mat = *matrix;
   ...
   void Manipulate(const glm::mat4 *view, const glm::mat4 *projection, OPERATION operation, MODE mode, glm::mat4 *matrix, float *deltaMatrix, float *snap, float *localBounds, float *boundsSnap)
   {
   ...
   void DrawCube(const glm::mat4 *view, const glm::mat4 *projection, const glm::mat4 *matrix)
   {
      matrix_t viewInverse;
   ...
   void DrawGrid(const glm::mat4 *view, const glm::mat4 *projection, const glm::mat4 *matrix, const float gridSize)
   {
      matrix_t res = *(matrix_t*)matrix * *(matrix_t*)view * *(matrix_t*)projection;
   ...
   void ViewManipulate(glm::mat4* view, float length, ImVec2 position, ImVec2 size, ImU32 backgroundColor)
   {
	  ...
	  glm::vec3 tempUp(up.x, up.y, up.z);
	  cubeView = glm::lookAt(CAMERA_MANAGER->GetCameraPosition(), glm::vec3(0), tempUp);
     // LookAt(&eye.x, &zero.x, &up.x, cubeView);
      ...
      if (interpolationFrames)
      {
		 ...
		 glm::vec3 tempEye(newEye.x, newEye.y, newEye.z);
		 glm::vec3 tempTarget(camTarget.x, camTarget.y, camTarget.z);
		 glm::vec3 tempUp(newUp.x, newUp.y, newUp.z);
		 *view = glm::lookAt(tempEye, tempTarget, tempUp);
      ...
      if (isDraging)
      {
		 ...
		 glm::vec3 tempEye(newEye.x, newEye.y, newEye.z);
		 glm::vec3 tempTarget(camTarget.x, camTarget.y, camTarget.z);
		 glm::vec3 tempUp(referenceUp.x, referenceUp.y, referenceUp.z);
		 *view = glm::lookAt(tempEye, tempTarget, tempUp);
         //LookAt(&newEye.x, &camTarget.x, &referenceUp.x, view);
      }
   }

   void EditTransform(const glm::mat4 *cameraView, const glm::mat4 *cameraProjection, glm::mat4* matrix, 
	   glm::vec3* newScale, glm::vec3* newRotate, glm::vec3* newTrans, bool *manupi, int *TRS, bool isBound)
   {
	   *manupi = true;
	   ...
	   if (*TRS == 0) mCurrentGizmoOperation = ImGuizmo::TRANSLATE;
	   if (*TRS == 1) mCurrentGizmoOperation = ImGuizmo::ROTATE;
	   if (*TRS == 2) mCurrentGizmoOperation = ImGuizmo::SCALE;
	   boundSizing = isBound;
	...
	/*   ImGui::Checkbox("Bound Sizing", &boundSizing);
	   ...
	   //SetDrawlist();
	   Manipulate(cameraView, cameraProjection, mCurrentGizmoOperation, mCurrentGizmoMode, matrix, NULL, useSnap ? &snap[0] : NULL, boundSizing ? bounds : NULL, boundSizingSnap ? boundsSnap : NULL);
	   DecomposeMatrixToComponents(matrix, (float*)newTrans, (float*)newRotate, (float*)newScale);
	   *manupi = false;
   }
};
